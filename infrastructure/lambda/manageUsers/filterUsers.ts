...
import { AttributeType, ListUsersCommandOutput } from "@aws-sdk/client-cognito-identity-provider";
...
export default function filterUsers(
  ...
  unfilteredUsers.Users!.forEach((user) => {
    ...
    if (tenant && tenant.Value === adminTenantId) {
      console.debug(`TenantId: ${JSON.stringify(tenant)}`);
      ...
      const lastName: AttributeType | undefined = user.Attributes!.find((attr) => {
      ...
      const firstName: AttributeType | undefined = user.Attributes!.find((attr) => {
