...
export class dt_translationTranslate extends Construct {
  ...
  constructor(scope: Construct, id: string, props: props) {
    ...
    const updateDbStatusProcessing = new tasks.DynamoUpdateItem(this, "updateDbStatusProcessing", {
      resultPath: sfn.JsonPath.DISCARD,
      table: props.jobTable,
      key: {
        id: tasks.DynamoAttributeValue.fromString(sfn.JsonPath.stringAt("$.jobDetails.jobId")),
      },
      updateExpression: "SET " + props.namedStrings.attributeForJobStatus + " = :value",
    ...
    const updateDbSourceKey = new tasks.DynamoUpdateItem(this, "updateDbSourceKey", {
      resultPath: sfn.JsonPath.DISCARD,
      table: props.jobTable,
      key: {
        id: tasks.DynamoAttributeValue.fromString(sfn.JsonPath.stringAt("$.jobDetails.jobId")),
      },
      updateExpression: "SET source" + props.namedStrings.keySuffix + " = :value",
      expressionAttributeValues: {
    ...
    const listTerminologies = new tasks.CallAwsService(this, "listTerminologies", {
    ...
    const parseTerminologiesLambdaRole = new iam.Role(this, "parseTerminologiesLambdaRole", {
      // ASM-L6 // ASM-L8
      assumedBy: new iam.ServicePrincipal("lambda.amazonaws.com"),
      description: "Lambda Role (Parse available Translate terminologies)",
    });
    const lambdaParseTerminologies = new dt_lambda(this, "lambdaParseTerminologies", {
      functionName: "ParseTerminologies",
      role: parseTerminologiesLambdaRole,
      path: "lambda/parseTerminologies",
      description: "Parse available Translate terminologies",
    });
    const parseTerminologies = new tasks.LambdaInvoke(this, "parseTerminologies", {
      ...
      payload: sfn.TaskInput.fromObject({
        terminologies: sfn.JsonPath.objectAt("$.listTerminologies.TerminologyPropertiesList"),
      }),
    });

    // STATE MACHINE | TRANSLATE | TASKS | isCustomTerminologyAvailable
    const isCustomTerminologyAvailable = new sfn.Pass(this, "isCustomTerminologyAvailable", {
    ...
    const updateDbPreTranslate = new tasks.DynamoUpdateItem(this, "updateDbPreTranslate", {
      resultPath: sfn.JsonPath.DISCARD,
      table: props.jobTable,
      key: {
        id: tasks.DynamoAttributeValue.fromString(sfn.JsonPath.stringAt("$.jobDetails.jobId")),
      },
      updateExpression: "SET " + props.namedStrings.mapForTranslateStatus + ".#attribute = :value",
    ...
    const setCustomTerminologyTrue = new sfn.Pass(this, "setCustomTerminologyTrue", {
      resultPath: "$.iterationDetails.setCustomTerminology",
      parameters: {
        Payload: sfn.JsonPath.array(
          sfn.JsonPath.format("{}", sfn.JsonPath.stringAt("$.iterationDetails.languageTarget")),
        ),
      },
    });

    // STATE MACHINE | TRANSLATE | TASKS | setCustomTerminologyTrue - Set Translate custom terminology to none for language
    const setCustomTerminologyFalse = new sfn.Pass(this, "setCustomTerminologyFalse", {
      resultPath: "$.iterationDetails.setCustomTerminology",
      ...
      parameters: {
        ...
        InputDataConfig: {
          ...
          S3Uri: sfn.JsonPath.format(
            "s3://" + props.contentBucket.bucketName + "/{}/" + props.namedStrings.s3StageUpload,
            sfn.JsonPath.stringAt("$.jobDetails.s3PrefixToJobId"),
          ),
        },
        OutputDataConfig: {
          S3Uri: sfn.JsonPath.format(
            "s3://" + props.contentBucket.bucketName + "/{}/" + props.namedStrings.s3StageOutput,
            sfn.JsonPath.stringAt("$.jobDetails.s3PrefixToJobId"),
          ),
        },
        "SourceLanguageCode.$": "$.jobDetails.languageSource",
        "TargetLanguageCodes.$": "States.Array($.iterationDetails.languageTarget)",
    ...
    const updateDbTranslateResultKey = new tasks.DynamoUpdateItem(
      ...
      "updateDbTranslateResultKey",
      {
        ...
        updateExpression: "SET " + props.namedStrings.mapForTranslateKey + ".#attribute = :value",
        expressionAttributeNames: {
          "#attribute": sfn.JsonPath.format(
            "{}{}",
            props.namedStrings.attributeSafePrefix,
    ...
    const updateDbTranslateResultStatus = new tasks.DynamoUpdateItem(
      ...
      "updateDbTranslateResultStatus",
      {
        ...
        updateExpression:
          "SET " + props.namedStrings.mapForTranslateStatus + ".#attribute = :value",
        expressionAttributeNames: {
          "#attribute": sfn.JsonPath.format(
            "{}{}",
            props.namedStrings.attributeSafePrefix,
            ...
            loopLangForTranslate.iterator(
              updateDbPreTranslate.next(isCustomTerminologyAvailable).next(
                ...
                new sfn.Choice(this, "useCustomTerminologyAvailable")
                  .when(
                    sfn.Condition.booleanEquals("$.isCustomTerminologyAvailable.result", true),
    ...
    NagSuppressions.addResourceSuppressions(
      ...
      [
        ...
        {
          id: "AwsSolutions-IAM5",
          reason: "Dynamic finding ids unknown at build time. Allow wildcard within account.",
          appliesTo: ["Resource::arn:aws:macie2:<AWS::Region>:<AWS::AccountId>:*"],
        },
        {
          id: "AwsSolutions-IAM5",
          reason:
            "Dynamic translation job id unknown at build time. Allow wildcard within account.",
          appliesTo: ["Resource::arn:aws:translate:<AWS::Region>:<AWS::AccountId>:*"],
        },
        {
          id: "AwsSolutions-IAM5",
    ...
    const policyPermitListTerminologies = new iam.Policy(this, "permitListTerminologies", {
    ...
    const policyPermitStartTranslationJob = new iam.Policy(this, "permitStartTranslationJob", {
      ...
      [
        {
          ...
          reason:
            "All resources have been selected for you because this service does not allow you to choose specific resources.",
          appliesTo: ["Action::translate:StartTextTranslationJob", "Resource::*"],
        },
      ],
      true,
    );

    const policyPermitPassTranslateRole = new iam.Policy(this, "permitPassTranslateRole", {
