...
export class dt_translate extends Construct {
  ...
  constructor(scope: Construct, id: string, props: props) {
    ...
    this.contentBucket.addLifecycleRule({
      ...
      noncurrentVersionExpiration: cdk.Duration.days(props.contentLifecycleDefault),
      abortIncompleteMultipartUploadAfter: cdk.Duration.days(props.contentLifecycleDefault),
    });
    ...
    props.identityPool.authenticatedRole.attachInlinePolicy(policyAuthenticatedPermitScopedS3);
    ...
    const policyAuthenticatedPermitQuickTextTranslation = new iam.Policy(
      ...
      {
        policyName: "Translate-Text-Comprehend-Detect-Dominant",
        statements: [
          new iam.PolicyStatement({
            // ASM-IAM // ASM-COG7
    ...
    // NA
    // OUTPUT
    const listJobs_output_item = new OutputType(`${FEATURE_PREFIX}_listJobs_output_item`, {
      definition: {
    ...
    const listJobs_output = new OutputType(`${FEATURE_PREFIX}_listJobs_output`, {
      definition: {
    ...
    // OUTPUT
    const createJob_output = new OutputType(`${FEATURE_PREFIX}_createJob_output`, {
      definition: {
        contentType: GraphqlType.string({ isRequired: true }),
        createdAt: GraphqlType.awsTimestamp(),
        id: GraphqlType.id({ isRequired: true }),
    ...
    props.apiSchema.addMutation(`${FEATURE_PREFIX}CreateJob`, createJobMutation);
    ...
    const featTranslationTranslate = new dt_translationTranslate(this, "featTranslationTranslate", {
      namedStrings,
    ...
    if (props.translationPii) {
      ...
      this.contentBucket.addLifecycleRule({
        ...
        noncurrentVersionExpiration: cdk.Duration.days(props.contentLifecyclePii),
        abortIncompleteMultipartUploadAfter: cdk.Duration.days(props.contentLifecyclePii),
      });
      // STATE MACHINE | PII
      const featTranslationPii = new dt_translationPii(this, "featTranslationPii", {
        namedStrings,
        contentBucket: this.contentBucket,
        jobTable: this.jobTable,
        removalPolicy: props.removalPolicy, // ASM-CFN1
      ...
      // STATE MACHINE | PII
      const featTranslationTag = new dt_translationTag(this, "featTranslationTag", {
        namedStrings,
        contentBucket: this.contentBucket,
        removalPolicy: props.removalPolicy, // ASM-CFN1
      });
    ...
    // STATE MACHINE | MAIN
    const featTranslationMain = new dt_translationMain(this, "featTranslationMain", {
      namedStrings,
      jobTable: this.jobTable,
      s3PrefixPrivate: props.s3PrefixPrivate,
      removalPolicy: props.removalPolicy, // ASM-CFN1
    ...
    NagSuppressions.addResourceSuppressionsByPath(
      cdk.Stack.of(this),
      `/${
        cdk.Stack.of(this).node.findChild("LogRetentionaae0aa3c5b4d4f87b02d85b201efdd8a").node.path
      }/ServiceRole/Resource`,
      [
        {
          id: "AwsSolutions-IAM4",
          reason: "CDK generates role with default policy. Default policy uses wildcard.",
        },
      ],
      true,
    );
