...
If you do not wish to use this feature you can skip the steps below.
...
If the following error occurs, this is expected when Macie is already enabled. It is safe to continue.
...
This command ensures that the Log Group exists if not already present. The solution will use the existing Log Group if it exists.

```sh
aws logs create-log-group --log-group-name /aws/macie/classificationjobs
```

If the following error occurs, this is expected when the Log Group already exists. It is safe to continue.
